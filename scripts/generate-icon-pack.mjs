...
const fallbackPngPath = path.join(publicDir, "Logo.png");
...
const getSourcePathForSize = (size) => {
  const explicitSizedPath = path.join(publicDir, `icon-${size}.png`);
  return explicitSizedPath;
};
...
const run = async () => {
  ...
  const pngBuffers = await Promise.all(
    iconSizes.map(async (size) => {
      const candidatePath = getSourcePathForSize(size);

      try {
        return await sharp(candidatePath)
          .resize(size, size, {
            fit: "contain",
        ...
        return await sharp(fallbackPngPath)
          .resize(size, size, {
            fit: "contain",
            background: { r: 0, g: 0, b: 0, alpha: 0 },
          })
